...
function solve_minlp_refgen(ROPF, fixing, index_var)
    instance = ROPF.instance_name
    generation = ROPF.generation
    root = pwd()
    # date = Dates.format(now(), "yy_u_dd_HH_MM_SS")
    isdir("knitro_runs") || mkpath("knitro_runs")
    if length(fixing) > 0
        outlog = joinpath(pwd(), "knitro_runs", "BB_$(instance)_$(generation).log")
    else
        outlog = joinpath(pwd(), "knitro_runs", "$(instance)_$(generation).log")
    end
    pb_path = ROPF.output_instance_path
    src_ampl_path = joinpath(pwd(), "src_ampl")
    ...
    close(f)
    ref_bus = readlines(joinpath(generation_files_path, "$(instance)_refbus.csv"))[1]
    f = open("refgen.dat", "w")
    write(f, ref_bus)
    close(f)
    ...
    if all_fixing
        open("phase3_refgen.run", "w") do f
            println(f, "include $(joinpath(src_ampl_path, "phase3_refgen.run"));")
        end
    else
        open("minlp_refgen.run", "w") do f
            println(f, "include $(joinpath(src_ampl_path, "minlp_refgen.run"));")
        end
    end

    open("minlp_refgen.mod", "w") do f
      println(f, "include $(joinpath(src_ampl_path, "minlp_refgen.mod"));")
    end

    try
        if all_fixing
            run(`cmd /c ampl phase3_refgen.run '>' $(outlog)`)
        else
            run(`cmd /c ampl minlp_refgen.run '>' $(outlog)`)
        end
        # run(`cmd /c ampl real_minlp.run `)
    catch
        @warn("AMPL/Knitro failed, returning.")
