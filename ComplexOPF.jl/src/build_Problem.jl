...
function build_Problem!(OPFpbs#=::OPFProblems=#, scenario::String)
  ...
  for bus in get_buses(OPFpbs, scenario)
    ...
    for (elemid, elem) in ds.bus[bus]
      ...
      # if elemid == "Shunt"
      #   println("creation of shunt var at bus $bus")
      # end
    end
  end
  # bus = "BUS_24"
  ...
  # bus_vars = mp.node_vars[bus]
  # elemid = "Shunt"
  # elem = bus_elems[elemid]
  # println(elem)
  # println(bus_vars["Volt"])
  # create_vars!(elem, bus, elemid, :NbMinVar, bus_vars, basecase_scenario_name())
  # println(bus_vars["Shunt"])
  # println(Snodal(elem, bus, elemid, :NbMinVar, bus_vars))
