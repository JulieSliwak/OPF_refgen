...
struct GOCShunt <: AbstractNodeLabel
    busid::Int64
    id::String #label
    shunt::Complex # coeffS = Gs + im*Bs
end

"""
    create_vars!(element::T, bus::String, elemid::String, elem_formulation::Symbol, bus_vars::SortedDict{String, Variable}, scenario::String) where T <: GOCShunt

Create shunt variable for shunt `elemid` at `bus` in `bus_vars` if `elem_formulation == :NewVar`. \n
Return nothing
"""
function create_vars!(element::T, bus::String, elemid::String, elem_formulation::Symbol, bus_vars::SortedDict{String, Variable}, scenario::String) where T <: GOCShunt
...
"""
  [cstrname, polynom, lb, ub] = Snodal(element::T, busid::String, elemid::String, elem_formulation::Symbol, bus_vars::SortedDict{String, Variable}) where T <: GOCShunt
...
"""
function Snodal(element::T, busid::String, elemid::String, elem_formulation::Symbol, bus_vars::SortedDict{String, Variable}) where T <: GOCShunt
    cstrname = "SHUNT"
...
"""
    constraint(element::T, bus::String, elemid::String, elem_formulation::Symbol, bus_vars::SortedDict{String, Variable}, scenario::String, OPFpbs::OPFProblems) where T <: GOCShunt
...
"""
function constraint(element::T, bus::String, elemid::String, elem_formulation::Symbol, bus_vars::SortedDict{String, Variable}, scenario::String, OPFpbs::OPFProblems) where T <: GOCShunt
    if elem_formulation == :NewVar
      cstrname = get_ShuntDef_cstrname()
      Sshunt = element.shunt* abs2(bus_vars[volt_name()])
      return SortedDict{String, Constraint}(cstrname => Polynomial(Sshunt - bus_vars[elemid]) == 0)
    end
