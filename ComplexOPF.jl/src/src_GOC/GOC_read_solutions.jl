...
function create_voltagesolutionpoint(bus_solution, index)
    ...
    for i in 1:size(bus_solution,1)
        ...
        if bus_solution[i,1] == 0
            MathProgComplex.add_coord!(point, Variable(variable_name("VOLT", bus_name(num_bus), "",basecase_scenario_name()), Complex), value)
        else
            id_contingency = bus_solution[i,1]
            scenario_name = scenarioname(id_contingency)
            MathProgComplex.add_coord!(point, Variable(variable_name("VOLT", bus_name(num_bus), "", scenario_name), Complex), value)
        end
    end
    return point
end
...
function gen_solution_basecase(basecase_generator_solution,index)
    ...
    for i in 1:size(basecase_generator_solution,1)
        ...
        MathProgComplex.add_coord!(point, Variable(variable_name("Sgen", bus_name(num_bus), generator_name(gen_id), basecase_scenario_name()), Complex), Pg + im*Qg)
    end
    return point
end
...
function gen_solution_contingencies(basecase_generator_solution,contingency_generator_solution,delta_solution, participation_factors, index)
    ...
    for i in 1:size(contingency_generator_solution,1)
        ...
        MathProgComplex.add_coord!(point, Variable(variable_name("Sgen", bus_name(num_bus), generator_name(gen_id), scenario_name), Complex), im*Qg)
    end
    ...
    for l in 1:size(delta_solution,1)
        ...
        for i in 1:size(basecase_generator_solution,1)
            ...
            MathProgComplex.add_coord!(point, var, Pg + delta_solution[l,2] * participation_factors[bus_name(num_bus)][generator_name(gen_id)])
        end
    end
    return point
end
...
function create_delta_solution(solution2)
    ...
    for i in 1:size(delta_solution,1)
        ...
        MathProgComplex.add_coord!(point, Variable(get_delta_varname(scenario),Real), delta_solution[i,2])
    end
    return point
end
    ...
    for (scenario, dict_modules) in module_v_scenarios
        for num_bus in generators
            ...
            if abs(dict_modules[num_bus]^2 - module_v_basecase[num_bus]^2) < get_GOC_Volt_ϵ()
                MathProgComplex.add_coord!(point, Variable(get_binEq_varname(scenario, basecase_scenario_name(), bus_name(num_bus)),Bool), 1.0)
                # MathProgComplex.add_coord!(point, Variable(get_binInf_varname(basecase_scenario_name(),scenario, bus_name(num_bus)),Bool), 1e-16)
                # MathProgComplex.add_coord!(point, Variable(get_binInf_varname(scenario, basecase_scenario_name(),bus_name(num_bus)),Bool), 1e-16)
            elseif dict_modules[num_bus]^2 - module_v_basecase[num_bus]^2 > get_GOC_Volt_ϵ()
                MathProgComplex.add_coord!(point, Variable(get_binInf_varname(basecase_scenario_name(),scenario, bus_name(num_bus)),Bool), 1.0)
                # MathProgComplex.add_coord!(point, Variable(get_binInf_varname(scenario, basecase_scenario_name(),bus_name(num_bus)),Bool), 1e-16)
                # MathProgComplex.add_coord!(point, Variable(get_binEq_varname(scenario, basecase_scenario_name(), bus_name(num_bus)),Bool), 1e-16)
            else
                MathProgComplex.add_coord!(point, Variable(get_binInf_varname(scenario, basecase_scenario_name(),bus_name(num_bus)),Bool), 1.0)
                # MathProgComplex.add_coord!(point, Variable(get_binInf_varname(basecase_scenario_name(),scenario, bus_name(num_bus)),Bool), 1e-16)
                # MathProgComplex.add_coord!(point, Variable(get_binEq_varname(scenario, basecase_scenario_name(), bus_name(num_bus)),Bool), 1e-16)
            end
        end
